...
logging.basicConfig(filename='pu_gaus.out', level=logging.INFO)
...
#m1 = np.distributions.multivariate_normal.MultivariateNormal(loc=mean1, covariance_matrix=cov1)
#m2 = np.distributions.multivariate_normal.MultivariateNormal(loc=mean2, covariance_matrix=cov2)

gaus1 = np.random.multivariate_normal(mean1, cov1, 10000)
gaus2 = np.random.multivariate_normal(mean2, cov2, 10000)
...
for epoch in range(n_epoch):
    ...
    if epoch % 1000 == 0:
        ...
        plt.savefig('./1/2gaus{}.png'.format(epoch))
